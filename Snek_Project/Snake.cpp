...
{
	for (int i = BOX_SIZE / (movementScale / 10); i < snake_lenght; ++i) {
...
bool game::Snake::ateFood (Food* fd)
{
	...
	if (checkCollision (body[0], fd->getFood ())) {
		updateLenght = true;
		sf::Vector2f new_location = body[snake_lenght - 1].getPosition ();
...
void game::Snake::moveSnake (sf::Vector2<int> direction)
{
	...
	int index = 0;
	while (i != snake_direction_list.end () && index < snake_lenght) {
